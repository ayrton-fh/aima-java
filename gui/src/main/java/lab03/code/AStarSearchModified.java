...
public class AStarSearchModified<A, S> extends AStarSearch<A, S> {
     ...
     * @param h
     */
    public AStarSearchModified(ToDoubleFunction<Node<A, S>> h) {
        super(h);
    }

    // expandade nodes
    List<Node<A, S>> expandedNodes = new ArrayList<>();
    ...
    public Node<A, S> search(Problem<A, S> problem) {
        ...
        while (!frontier.isEmpty() && (solution == null || getCostValue(frontier.peek()) < getCostValue(solution))) {
            Node<A, S> parent = frontier.poll();

            // compute expanded nodes
            ...
            for (Node<A, S> child : SearchUtils.successors(problem, parent)) {
                ...
                if (!reached.containsKey(s) || getCostValue(child) < getCostValue(reached.get(s))) {
                    reached.put(s, child);
                    frontier.add(child);

                    // if child is a goal and is cheaper than solution
                    if (problem.isGoalState(s) && (solution == null || getCostValue(child) < getCostValue(solution))) {
                        solution = child;
                    }
                }
            }
